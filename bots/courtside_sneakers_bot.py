...
sizes = ['9.5']
new_arrivals_page_url = 'https://www.courtsidesneakers.com/collections/new-releases'
base_url = 'https://www.courtsidesneakers.com'
keywords = ['fiba']
...
def get_product_page_url():
    for retries in range(15):
        ...
        href_link = soup.find(
            "a", {'href': re.compile("|".join(keywords))})
...
def checkout_page(product_page_url):
    ...
    for option in options:
        if size_collected == len(sizes):
            break
        elif option.get('value') in sizes:
            continue
        else:
            if re.match
                id = option.get('value')
